# Addendum: Canonical Flow, Compatibility Audit, and New Documentation

## 🔄 Module Renaming Update (August 2024)

All files in the `canonical_flow` directory have been renamed to follow Python module naming conventions:

**Key Changes:**
- Removed numeric prefixes from all files (01I_, 02A_, etc.)
- Replaced with descriptive alphabetic names
- Updated all import statements throughout the codebase
- Maintained backward compatibility through updated documentation

**Examples:**
- `01I_pdf_reader.py` → `pdf_reader.py`
- `13A_adaptive_analyzer.py` → `adaptive_analyzer.py`
- `math_stage01_ingestion_enhancer.py` → `ingestion_enhancer.py`

This repository now includes an automated canonicalization step and a compatibility auditor.
...
Artifacts written to canonical_flow/:
- readiness.json — entry point / dependencies / tests flags
- compilation_report.json — Python byte-compilation report
- project_analysis_report.json — structure/imports/dependency snapshot
- dependency_audit.json — declared vs used libraries, duplicates, conflict heuristics

Additional documentation:
- README_INNOVATION.md — innovation and differentiators
- README_CONTRACTS.md — contract suite (Routing, Snapshot, Conformal Risk, MCC/BMC, PIC/FFC) and validation commands

Note on spaCy models: install language models explicitly (extras do not install models):

```bash
python -m spacy download en_core_web_sm
```

The compatibility auditor is non-fatal; it highlights missing third‑party packages referenced in modules (e.g., whoosh, pyserini, pandas, ray, redis, kafka, sqlalchemy, pm4py, deap, kubernetes, httpx, pillow, pymupdf, opentelemetry, aioredis, textstat, unidecode, ftfy, camelot, tabula‑py, pdfplumber, pytesseract, easyocr, langdetect, torch_geometric, node2vec, etc.).
